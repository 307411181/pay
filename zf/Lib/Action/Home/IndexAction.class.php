...
class IndexAction extends Action {
    ...
    public function Index(){
	 ...
	 //  echo($str."ddddddd");
	   if($str != ""){
		   $str = trim($str,"&");
		   $str = "http://". C("WEB_APIURL") ."/Payapi_Index_Pay.html?".$str;	
		   $this->gheader($str);
		 // header("Location: $str")；
		  // exit();
	   }
 ...
 public function dborder(){
			...
			if(md5($_POST["verify"]) != session("verify")){
				...
				if($id){
				    ...
				    if($_POST["ajaxkey"] == "ok"){
						...
						//$EmailHTML = "亲爱的会员：".$this->_post("UserName")."您好！ <br>感谢您注册".C("WEB_NAME")."账户！<br>您现在可以激活您的".C("WEB_NAME")."账户，激活成功后，您可以使用".C("WEB_NAME")."提供的各种支付服务。 <br><a href='http://". C("WEB_URL") ."/Index_Activate_id_".$id."_activate_".$key.".html'>点此激活".C("WEB_NAME")."账户 </a><br>如果上述文字点击无效，请把下面网页地址复制到浏览器地址栏中打开：<br>http://". C("WEB_URL") ."/Index_Activate_id_".$id."_activate_".$key.".html<br>此为系统邮件，请勿回复<br>请保管好您的邮箱，避免".C("WEB_NAME")."账户被他人盗用<br>如有任何疑问，可查看 ".C("WEB_NAME")."相关规则，".C("WEB_NAME")."网站访问 http://". C("WEB_URL")."/<br>Copyright ".C("WEB_NAME")." 2013 All Right Reserved";
						//$ReturnEamil = $Email->SendEmail($this->_post("UserName"),C("WEB_NAME")."账户激活邮件！",$EmailHTML);
						//if($ReturnEamil=="1")
						//{
						    session("jh_username",$this->_post("UserName"));
							exit ("http://". C("WEB_URL") ."/Index_Activate_id_".$id."_activate_".$key.".html");
						//}else{
							//exit ($ReturnEamil);
						//}
	...
	public function regtwo(){
		...
		if($result_ko){
			...
			echo "<script>alert('注册成功!请登陆后点击审核信息菜单填写相关信息后提交审核后,联系客服为您审核.');location.href='http://". C("WEB_URL") ."/index_login.html'</script>";
			//  echo "chenggong ";
		}else{
			  echo "reg fail ";
		}
	...
	public function Activate(){
		...
		if($list != null){
			...
			if($row){
                ...
                echo '账号已激活！<a href="/index_login.html">立即登录</a>';
